...
# Discharge column names in priority order (raw USGS / CDEC downloads).
FLOW_COL_ORDER: Tuple[str, ...] = (
    "00060_Mean",
    "00060_2_Mean",
    "00054_Observation at 24:00",
)


def detect_flow_col(cols: Iterable[str]) -> str | None:
    """Return the first recognised discharge column name, or ``None``."""
    col_set = set(cols)
...
def water_year(date: str | pd.Timestamp) -> int:
    """Return the water year (Oct–Sep) for *date*."""
