...
| `single_lstm_kfold.csv` | Per-basin evaluation metrics for the single LSTM baseline 5-fold cross-validation |
| `vic_simulated.csv` | Per-basin evaluation metrics for VIC simulated runoff vs observed flow (computed from daily timeseries) |
| `cdf_nse.png` | CDF plot of NSE across all models |
| `cdf_kge.png` | CDF plot of KGE across all models |
| `cdf_fhv.png` | CDF plot of FHV (%) — peak flow bias |
| `cdf_flv.png` | CDF plot of FLV (%) — low flow bias |
| `cdf_kge_vic_comparison.png` | CDF plot of KGE comparing LSTM vs VIC calibrated/regionalized on overlapping basins |
